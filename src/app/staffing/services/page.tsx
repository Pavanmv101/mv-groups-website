...
// Only staffing-related services (exclude event_management)
const STAFFING_SERVICES = SERVICES.filter(s => s.id !== 'event_management');
  ...
  return (
    <div style={{ background: '#0c0b0a', minHeight: '100vh' }}>
      ...
      <section className="relative pt-32 pb-16 lg:pt-48 lg:pb-24 overflow-hidden border-b" style={{ borderBottomColor: '#1a1918' }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <span style={{ color: '#f3c892' }}>Event Crew</span>
          </h1>
          <p
          ...
          >
            ...
            Groups delivers tailored manpower supply across Karnataka.
          </p>
        </div>
      </section>
      ...
      <div className="relative">
        {STAFFING_SERVICES.map((service, idx) => {
...
function ServiceDetailSection({ service, Icon, reversed, index, bgImage }: ServiceDetailProps) {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`grid lg:grid-cols-12 gap-12 lg:gap-16 items-start ${reversed ? 'lg:flex-row-reverse' : ''}`}>
          <div className={`lg:col-span-6 lg:sticky lg:top-32 ${visible ? 'animate-fade-in-up' : 'opacity-0'} ${reversed ? 'lg:order-2' : 'lg:order-1'}`}>
            <div
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h2 className="text-3xl lg:text-4xl font-bold mb-4" style={{ color: '#ffffff' }}>{service.title}</h2>
                <p className="text-lg leading-relaxed" style={{ color: '#a39e98' }}>{service.description}</p>
              </div>
            </div>
          </div>

          <div className={`lg:col-span-6 lg:pt-10 space-y-10 ${visible ? 'animate-fade-in-up delay-200' : 'opacity-0'} ${reversed ? 'lg:order-1' : 'lg:order-2'}`}>
            <div>
              <span className="inline-block px-5 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-8" style={{ background: '#1a1918', color: '#66625d', border: '1px solid #282624' }}>
                Key Capabilities
              </span>
              <ul className="space-y-6">
                {service.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start gap-4 group p-4 rounded-2xl transition-all duration-300 hover:bg-[#1a1918]" style={{ border: '1px solid transparent' }}>
                    <CheckCircle2 className="w-6 h-6 shrink-0 mt-0.5 transition-all duration-500 group-hover:scale-110 group-hover:text-white" style={{ color: '#f3c892' }} />
                    <span className="text-lg leading-snug transition-colors duration-300 group-hover:text-[#f3c892]" style={{ color: '#ffffff' }}>{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="pt-8 flex flex-col sm:flex-row gap-4 border-t" style={{ borderColor: '#282624' }}>
              <Link href={`/services/${service.id}`} className="inline-flex items-center justify-center px-8 py-4 rounded-full font-bold text-sm transition-all hover:bg-white hover:text-black group" style={{ border: '1px solid #282624', color: '#ffffff', background: 'transparent' }}>
                Learn More
              ...
              <Link href={`/booking?service=${service.id}`} className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full font-bold text-sm transition-transform hover:-translate-y-1 shadow-lg" style={{ background: 'linear-gradient(135deg, #f3c892, #d4aa73)', color: '#0c0b0a' }}>
                Book Your Team
              </Link>
            </div>
          </div>
