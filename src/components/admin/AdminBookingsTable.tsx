    ...
    <div className="bg-[#141312] rounded-2xl shadow-sm border border-[#282624] overflow-hidden">
      <div className="px-6 py-5 border-b border-[#282624] bg-[#141312]/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-lg font-semibold text-white flex items-center gap-2">
          <Filter className="w-5 h-5 text-[#66625d]" />
          Filter Bookings
        </h2>
      ...
      {selectedIds.length > 0 && (
        <div className="bg-indigo-50 px-6 py-3 border-b border-indigo-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={handleBulkApprove} className="px-3 py-1.5 bg-[#141312] border border-indigo-200 text-indigo-700 rounded shadow-sm text-sm font-medium hover:bg-indigo-50">Approve Selected</button>
            <button onClick={handleExportCSV} className="px-3 py-1.5 bg-[#141312] border border-indigo-200 text-indigo-700 rounded shadow-sm text-sm font-medium hover:bg-indigo-50 flex items-center gap-1"><Download className="w-4 h-4"/> Export CSV</button>
          </div>
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#0a0908] border-b border-[#282624] text-[#66625d] text-sm">
              <th className="px-6 py-4 font-semibold w-12">
                <input type="checkbox" checked={paginatedBookings.length > 0 && selectedIds.length === paginatedBookings.length} onChange={handleSelectAll} className="rounded border-[#403e3c] text-indigo-600 focus:ring-indigo-500"/>
              </th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedBookings.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-[#66625d]">
                  No bookings found for the selected filter.
                </td>
              </tr>
            ) : (
              paginatedBookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-[#141312]/50 transition-colors">
                  <td className="px-6 py-4 align-top">
                    <input type="checkbox" checked={selectedIds.includes(booking.id)} onChange={() => toggleSelect(booking.id)} className="rounded border-[#403e3c] text-indigo-600 focus:ring-indigo-500"/>
                  </td>
                  <td className="px-6 py-4 align-top">
                    <p className="font-semibold text-white">{booking.contact_name}</p>
                    <p className="text-sm text-[#66625d]">{booking.contact_email}</p>
                    <p className="text-sm text-[#66625d]">{booking.contact_phone}</p>
                  </td>
                  <td className="px-6 py-4 align-top">
                    <p className="font-medium text-white">{getServiceName(booking.service_type)}</p>
                    <p className="text-xs text-[#66625d] mt-1">ID: {booking.id.split('-')[0]}</p>
                  </td>
                  <td className="px-6 py-4 align-top">
                    <p className="text-sm text-[#c8c3be] flex items-center gap-1">
                      <Calendar className="w-3.5 h-3.5 text-[#403e3c]" />
                      {new Date(booking.start_date).toLocaleDateString()} - {new Date(booking.end_date).toLocaleDateString()}
                    </p>
                    <p className="text-sm text-[#c8c3be] flex items-center gap-1 mt-1">
                      <Users className="w-3.5 h-3.5 text-[#403e3c]" />
      ...
      {totalPages > 1 && (
        <div className="px-6 py-4 border-t border-[#282624] flex items-center justify-between bg-[#0a0908]">
          ...
          <div className="flex gap-2">
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded border border-[#282624] bg-[#141312] text-[#a39e98] disabled:opacity-50 hover:bg-[#0a0908]">
              <ChevronLeft className="w-4 h-4"/>
            </button>
            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded border border-[#282624] bg-[#141312] text-[#a39e98] disabled:opacity-50 hover:bg-[#0a0908]">
