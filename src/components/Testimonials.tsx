          ...
          {displayTestimonials.map((t, idx) => (
            ...
            >
              ...
              <div className="flex gap-1 mb-4">
                {STARS.map((s) => (
                  <svg key={s} className="w-4 h-4" viewBox="0 0 20 20" fill="#f3c892">
              ...
              {/* Quote */}
              <p className="text-sm leading-relaxed flex-grow mb-6" style={{ color: '#c8c3be' }}>
                &ldquo;{t.quote}&rdquo;
              </p>

              {/* Author */}
              <div className="flex items-center gap-3 pt-5" style={{ borderTop: '1px solid #282624' }}>
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0"
                  style={{ background: 'rgba(243,200,146,0.15)', color: '#f3c892', border: '1px solid rgba(243,200,146,0.3)' }}
                >
                  {t.client_name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h4 className="font-bold text-sm" style={{ color: '#f3c892' }}>
                    {t.client_name}
                  </h4>
